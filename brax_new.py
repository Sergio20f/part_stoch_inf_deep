...
from sdeintv2 import sdeint_ito_fixed_grid
...
def PSDEBNN(fx_block_type,
    ...
    def make_layer(input_shape):
        ...
        def _apply_fun_sde_first(params, inputs, rng, full_output=False, fixed_grid=True, **kwargs):
            ...
            if fixed_grid:
                ys = sdeint_ito_fixed_grid(f_aug_sde, g_aug_sde, y0, ts_sde, rng, fw_params, method="euler_maruyama", rep=rep, ode_method='euler', start_idx=x_dim, end_idx=x_dim+w_dim)
        ...
        def _apply_fun_ode_first(params, inputs, rng, full_output=False, fixed_grid=True, **kwargs):
            ...
            if timecut > 0.0:
                if fixed_grid:
                    ys = sdeint_ito_fixed_grid(f_aug_sde, g_aug_sde, y0_sde, jnp.linspace(0, timecut, nsteps-s+1), rng, fw_params, method="euler_maruyama", rep=rep, ode_method='euler', start_idx=x_dim, end_idx=x_dim+w_dim)
...
def PSDEBNN_H(fx_block_type,
    ...
    def make_layer(input_shape):
        ...
        def apply_fun(params, inputs, rng, full_output=False, fixed_grid=True, **kwargs):
            ...
            rep = w_dim if stl else 0  # STL
            if fixed_grid:
                ys = sdeint_ito_fixed_grid(f_aug, g_aug, y0, ts, rng, (fw1_params, fw2_params), method="euler_maruyama", rep=rep, ode_method='euler', start_idx=x_dim, end_idx=x_dim+w_dim1)
            else:
                print("using stochastic adjoint")
