    ...
    def make_layer(input_shape):
      ...
      def f_aug_ode(y, t, args):
        ...
        def _apply_fun_sde_first(params, inputs, rng, full_output=False, fixed_grid=True, **kwargs):
            ...
            x = inputs
            if infer_initial_state:
                raise ValueError("infer_initial_state not implemented for PSIDBNN - Ask Francesco")
            ...
            if fixed_grid:
                ys = sdeint_ito_fixed_grid(f_aug, g_aug, y0, ts_sde, rng, fw_params, method="euler_maruyama", rep=rep)
            
            else:
                print("using stochastic adjoint")
                ys = sdeint_ito(f_aug, g_aug, y0, ts_sde, rng, fw_params, method="euler_maruyama", rep=rep)

            ys2 = ys[-1]  # Take last time value.
            kl = kl + jnp.sum(y[x_dim + w_dim:])

            ys3 = ys2[:(x_dim+w_dim))]
            ys4 = odeint(f_aug_ode, ys2, ts_ode, args=(fw_params,), method="euler")
        ...
        def _apply_fun_ode_first(params, inputs, rng, full_output=False, fixed_grid=True, **kwargs):
            ...
            ys3 = ys2[:(x_dim+w_dim))]
            ys4 = odeint(f_aug_ode, ys3, ts_ode, args=(fw_params,), method="euler")
